locals {
  lb_netwrking = format("%s%s%s%s", "C", upper(var.tags["ib:resource:letter-ev"]), "AME2IDNLB", "00016")
}
  ...
  target_groups = [
    {
      ...
      targets = [
        {
          target_id         = "10.97.23.196"
          port              = 80
          availability_zone = "eu-west-1a"
        },
        {
          target_id         = "10.97.23.213"
          port              = 80
